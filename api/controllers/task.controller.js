...
exports.create = async (req, res) => {
    const { created_by, task_start_date, task_end_date, course_id, task_name } = req.body;
    if (created_by && task_start_date && task_end_date && course_id && task_name) {
        ...
        if (req.files) {
            if (req.files.file_name) {
                ...
                var newPath = path.join(process.cwd(), 'uploads/task_files', req.files.file_name.name)
                var newFileName = req.files.file_name.name.split(".zip");
                var newDirPath = path.join(process.cwd(), 'uploads/task_files', newFileName[0])
                req.files.file_name.mv(newPath);
...
exports.update = async (req, res) => {
    const {  task_start_date, task_end_date, course_id, id, task_status, task_name } = req.body;
